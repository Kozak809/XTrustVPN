...
import com.kozak.xtrustvpn.plugin.PluginManager.loadString
import com.kozak.xtrustvpn.utils.PackageCache

object Plugins {
    const val AUTHORITIES_PREFIX_SEKAI_EXE = "com.kozak.xtrustvpn.plugin."
    const val AUTHORITIES_PREFIX_NEKO_EXE = "moe.matsuri.exe."

    const val ACTION_NATIVE_PLUGIN = "com.kozak.xtrustvpn.plugin.ACTION_NATIVE_PLUGIN"

    const val METADATA_KEY_ID = "com.kozak.xtrustvpn.plugin.id"
    const val METADATA_KEY_EXECUTABLE_PATH = "com.kozak.xtrustvpn.plugin.executable_path"
    ...
    private fun getExtPluginOld(pluginId: String): List<ProviderInfo> {
        ...
        val list1 = SagerNet.application.packageManager.queryIntentContentProviders(
            Intent(ACTION_NATIVE_PLUGIN, buildUri(pluginId, "com.kozak.xtrustvpn")), flags
        )
